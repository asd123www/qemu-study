...
launch_src_vm() {
    ...
    local mem=$2
    screen -dmS "$session" \
        ./apps/controller shm src apps/vm-boot/redis.exp 4 "$mem" vm_src.txt 100000
}
...
for mem in "${MEM_SIZES[@]}"; do
    ...
    screen -dmS "$DST_SESSION" \
        ./apps/controller shm dst 4 "$mem" vm_dst.txt 1342177280B
    sleep 10

    # -------- Optional backup VM ---------------------------------------------
    screen -dmS "$BAK_SESSION" bash -c "./apps/controller shm backup 100 > fm2_redis_downtime_${WKLD}_${mem}.dat 2>&1"

    # -------- Workload --------------------------------------------------------
    ./redis_load.sh "$WKLD"
    sleep 30

    { ./redis_run.sh "$WKLD" | tee "fm2_redis_perf_wkld_${WKLD}_${mem}.dat"; } &
    redis_run_pid=$!
    sleep 30

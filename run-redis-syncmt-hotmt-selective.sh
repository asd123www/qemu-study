...
#    screen -dmS "$BAK_SESSION" ./apps/controller shm backup 30
    screen -dmS "$BAK_SESSION" bash -c "./apps/controller shm backup 100 > fm2+syncmt+hotmt+sel_redis_downtime_${wkld}.dat 2>&1"

    # -------- Workload --------------------------------------------------------
    ./redis_load.sh "$wkld"
    sleep 10

    { ./redis_run.sh "$wkld" | tee "fm2+syncmt+hotmt+sel_redis_perf_wkld_${wkld}.dat"; } &
    redis_run_pid=$!
    sleep 10
    ...
    sudo ./promo_hot_new_hotmt "$vm_pid" /dev/shm/my_shared_memory 2 0 >"/tmp/promo_${wkld}.log" 2>&1 &
    promo_pid=$!

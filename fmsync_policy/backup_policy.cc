...
void print_page_history(int page, int iter) {
    cout << "evict page: " << page << endl;
    for (int i = 1; i <= iter; i++) {
        cout << a[i][page];
    }
    printf(" ");
    for (int i = iter + 1; i <= 15; i++) {
        cout << a[i][page];
    }
    cout << endl;
}
    ...
    for (int iter = 1; iter <= 7; iter++) {
        ...
        while (dirty_set.size() > dirty_target) {
            ...
            for (int page = 0; page < m; ++page) {
                ...
                // printf("page: %d, next_use: %d\n", page, next_use[page].empty() ? 1000000 : next_use[page].front());
            ...
            for (int i = 0; i < dirty_set.size() - dirty_target; ++i) {
                ...
                printf("min_page: %d, min_distance: %d\n", min_page, min_distance);
                print_page_history(min_page, iter);

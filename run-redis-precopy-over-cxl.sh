...
#    screen -dmS "$BAK_SESSION" ./apps/controller shm backup 30
    screen -dmS "$BAK_SESSION" bash -c "./apps/controller shm backup 150 > precopy-cxl_redis_downtime_100G_${wkld}.dat 2>&1"

    # -------- Workload --------------------------------------------------------
    ./redis_load.sh "$wkld"
    sleep 10

    { ./redis_run.sh "$wkld" | tee "precopy-cxl_redis_perf_wkld_100G_${wkld}.dat"; } &
    redis_run_pid=$!
    sleep 10
